...
class FaceSwappers:
    def __init__(self, models_processor: 'ModelsProcessor'):
        ...
        self._sharpness_kernel = None  # Cached Gaussian kernel for unsharp mask
    ...
    def recognize(self, arcface_model, img, face_kps, similarity_type):
        if similarity_type == 'Optimal':
            ...
            #tform = trans.SimilarityTransform.from_estimate(face_kps, self.models_processor.arcface_dst)
            # Transform
            img = v2.functional.affine(img, tform.rotation*57.2958, (tform.translation[0], tform.translation[1]) , tform.scale, 0, center = (0,0) )
    ...
    def apply_sharpness(self, output_tensor: torch.Tensor, sharpness: float) -> torch.Tensor:
        """Apply sharpening to a swapped face tensor using unsharp masking.

        Args:
            output_tensor: Face tensor of shape (1, 3, H, W) in float32.
            sharpness: Sharpening strength. 0.0 = no effect, recommended range 0.1–2.0.

        Returns:
            Sharpened tensor of the same shape.
        """
        import torch.nn.functional as F

        # Build and cache the kernel once (or if device changes)
        if self._sharpness_kernel is None or self._sharpness_kernel.device != output_tensor.device:
            coords = torch.arange(3, dtype=torch.float32, device=output_tensor.device) - 1
            g = torch.exp(-(coords ** 2) / 2.0)
            kernel_1d = g / g.sum()
            kernel_2d = (kernel_1d[:, None] * kernel_1d[None, :]).expand(3, 1, 3, 3).contiguous()
            self._sharpness_kernel = kernel_2d

        blurred = F.conv2d(output_tensor, self._sharpness_kernel, padding=1, groups=3)
        return output_tensor + sharpness * (output_tensor - blurred)
    ...
    def run_inswapper(self, image, embedding, output, sharpness: float = 0.0):
        ...
        if sharpness != 0.0:
            sharpened = self.apply_sharpness(output.reshape(1, 3, 128, 128), sharpness)
            output.copy_(sharpened.reshape_as(output))
...
#=================================================================================================================        
    def calc_swapper_latent_hyperswap(self, source_embedding):
        """Calculate latent for HyperSwap model"""
        print(f"[DEBUG] FaceSwappers.calc_swapper_latent_hyperswap: Начало, embedding shape: {source_embedding.shape}")
        
        # HyperSwap НЕ использует emap (как CSCS/GhostFace/SimSwap)
        # Просто нормализуем эмбеддинг
        n_e = source_embedding / l2norm(source_embedding)
        latent = n_e.reshape((1, -1))
        
        print(f"[DEBUG] FaceSwappers.calc_swapper_latent_hyperswap: Результат, форма: {latent.shape}")
        return latent

    def run_hyperswap(self, image, embedding, output):
        """Run HyperSwap model inference"""
        print(f"[DEBUG] FaceSwappers.run_hyperswap: Начало, image: {image.shape}, embedding: {embedding.shape}")
        
        if not self.models_processor.models['HyperSwap_1C_256']:
            self.models_processor.models['HyperSwap_1C_256'] = self.models_processor.load_model('HyperSwap_1C_256')
        
        model = self.models_processor.models['HyperSwap_1C_256']
        
        # Проверяем входы/выходы модели
        inputs = model.get_inputs()
        outputs = model.get_outputs()
        print(f"[DEBUG] FaceSwappers.run_hyperswap: Входы модели: {[(i.name, i.shape) for i in inputs]}")
        print(f"[DEBUG] FaceSwappers.run_hyperswap: Выходы модели: {[(o.name, o.shape) for o in outputs]}")
        ...
        # Bind BOTH outputs: output и mask
        ...
        # Создаем тензор для маски
        mask_tensor = torch.empty((1, 1, 256, 256), dtype=torch.float32, device=self.models_processor.device).contiguous()
        io_binding.bind_output(
            name='mask', 
            device_type=self.models_processor.device, 
            device_id=0, 
            element_type=np.float32, 
            shape=(1, 1, 256, 256), 
            buffer_ptr=mask_tensor.data_ptr()
        )

        print(f"[DEBUG] FaceSwappers.run_hyperswap: Запуск модели на устройстве: {self.models_processor.device}")
        ...
        print("[DEBUG] FaceSwappers.run_hyperswap: Успешно выполнено")
        
        # Возвращаем маску для использования в frame_worker
        return mask_tensor